...
public class AES implements IEncrypt {
	...
	// Source https://github.com/xesco/CriptoFIB/blob/master/aes.java
	// I copied the code for the sbox lookup table because I didn't understand how
	// to generate it and did not want to type it by hand :(
	// Initializes sbox and sboxInv
	// @formatter:off
		final byte[][] sbox = { {(byte) 0x63, (byte) 0x7c,
		...
		final byte[][] sboxInv = { {(byte) 0x52, (byte) 0x09,
	...
	private byte[][] keySchedule; // TODO

	public AES() {
		keySchedule = new byte[32][4];
	}

	@Override
	public String encrypt(String plaintext) {
		String state = String.format("%040x", new BigInteger(1, plaintext.getBytes()));
		// Create 2D array of hexadecimals (each hexadecimal's length is 2; parse like
		// that)
		String[][][] blocks = new String[state.length() / 16 + 1][4][4];
		for (int i = 0; i < state.length(); i += 16) {
			blocks[i / 16] = toBlock(state.substring(i, i + 32));
	...
	private String[][] toBlock(String state) {
		...
		for (int c = 0; c < 4; c++) {
			for (int r = 0; r < 4; r++) {
				block[r][c] = state.substring(8 * c + 2 * r, 2);
			}
		}
	...
	/**
	 * Private helper method that goes through all the rounds given a block
	 * 
	 * @param String[][] original block
	 * @return String[][] encrypted block
	 */
	private String[][] encrypt(String[][] state) {
	...
	private void subBytes(String[][] state) {
		for (int r = 0; r < 4; r++) {
			for (int c = 0; c < 4; c++) {
				byte n = Byte.parseByte(state[r][c]);
				state[r][c] = String.valueOf(this.sbox[n % 256 - (n % 16)][n % 16]);
			}
		}
	}
	...
	private void addRoundKey(String[][] state) {
	}

	private void initKeySchedule() {
