...
/* I think the most helpful debugging tool is the Variables panel.
 * I like looking at all the variables' values.
 */
public class Guess2 {
	...
	public static void main(String[] args) {
		...
		for (int r = 1; r <= 5; r++) {
			...
			// BUG: rolled number can be 0
			// FIX: ranGen.nextInt(sides) + 1
			...
			// BUG: does not reset number of guesses per round
			// FIX: numGuesses = 0;
			numGuesses = 0;
			rightGuess = false;
			// BUG: keeps running after 3 guesses if no right guess
			// FIX: change || to &&
			while (numGuesses < 3 && !rightGuess) {
				...
				do {
					...
					// BUG: only checks invalid input once
					// FIX: change if to while

					// BUG: impossible for number to be < 1 and > 6
					// FIX: ||
					while ((userguess < 1) || (userguess > 6)) {
						System.out.println("   Please enter a valid guess [1-6]!");

						// BUG: does not take in new input
						// FIX: scanner update
						userguess = userInput.nextInt();
					}
			...
			if (!rightGuess) {
				// BUG: computer gets points if you got the right guess
				// FIX: flip test case to !rightGuess
				computerPoints++;
			} else {
				humanPoints++;
			}
